...
2026-06-02 12:17 JSTに、Vercel本番へAzure未投入時のCloudflare Worker live schema委譲を追加し、deployment `dpl_HMAXzQgody4kr7zCQ6CLSCHo97US` をproduction aliasへ反映した。`npm run check:seo -- --live` は引き続きブロッカー0で、`npm run check:production:strict-ai` もブロッカー0/警告0になった。SEOページ自体のURL、canonical、sitemap、Free月20回表記、Checkout導線は維持している。
